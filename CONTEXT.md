...
首次项目使用先完成统一就绪门禁，咨询、比较、指定模块、推荐和交付均不例外。门禁检查选定Python及锁定依赖、模型能力、iFind数据API和安装目录之外的项目Store；模型由Host托管时也必须显示为已配置，独立批处理则必须完整配置兼容模型端点。门禁通过后按任务复用已确认事实和合格DataAssetRef，不重复索取模型或数据凭据；已有结果交付不重新取数。
