...
DataAsset属于同一用户与租户内的全局数据资料库，不属于研究任务。任务和模块可以复用满足标的、字段、价格口径、交易日历与覆盖区间要求的DataAsset；每次运行仍须冻结并记录实际使用的精确DataAssetRef。
...
**合同结算收益率**：一笔完整历史合同在OptionReg已声明的全部合同现金流之和除以该产品合同标准化基准。已声明的期权费属于合同现金流；资金成本、交易费、税费、对冲和滑点不属于该指标。它不是客户资本回报率或客户净收益率。

**历史正收益样本占比**：完整且有效的历史合同样本中，合同结算收益率严格大于零的样本数除以有效样本总数。它只描述本次历史覆盖，不代表未来获利概率。

**回测日期预检**：在正式运行前，使用同一合同编译、历史行情、交易日历和完整期限规则计算可运行入场窗口的只读检查。预检不修改合同、不提交结果，也不得静默改写用户日期。

**合同期限与观察日数**：`T`按ACT/365保留合同和现金流时间语义；`n_obs`表示观察日数量。两者可以存在约束关系，但不得自动互相替代。修改`T`必须重算到期日、观察日程和路径适用性。
