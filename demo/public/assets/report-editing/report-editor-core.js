(() => {
  ...
  class ReportEditorController {
    ...
    _markDirty() {
      if (this.isDirty()) this._setStatus('有未保存修改');
    }
    ...
    replaceSelectedImage(file) {
      ...
      reader.addEventListener('load', () => {
        ...
        this._checkpoint();
        this._setStatus('图片已替换，保存副本后写入报告');
      });
    ...
    renderOutline() {
      ...
      this.nodes.editorOutline.replaceChildren(...headings.map(heading => {
        ...
        button.addEventListener('click', () => heading.scrollIntoView({block: 'start', behavior: 'smooth'}));
        return button;
      }));
    ...
    replaceAll() {
      ...
      this.renderOutline();
      this._checkpoint();
      this._setStatus(`已替换${matches.length}处，保存副本后写入报告`);
    ...
    async submit(format) {
      ...
      this.saving = true;
      this._setSubmitDisabled(true);
      this._setStatus(format === 'html' ? '正在保存副本' : format === 'pdf' ? '正在导出PDF' : '正在导出Word');
      try {
        ...
        const label = format === 'html' ? '副本已保存' : format === 'pdf' ? 'PDF已生成' : 'Word已生成';
        this._setStatus(stillCurrent ? `${label}，可从报告库再次编辑` : `${label}，当前仍有新修改`, stillCurrent ? 'saved' : '');
