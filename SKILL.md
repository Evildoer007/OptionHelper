...
`optionlib.md`是损益和公式的唯一依据。图示可辅助说明，但不能替代正文中的条款和损益逻辑。“待录入”表示尚未完成正式入库，可能缺定价资源或一致性核验，也可能已有待核验的候选完整正文和损益图；必须如实说明现有资料范围，不得自行补造条款。
...
1. 在`optionlist.md`确认结构是否已登记及正式入库状态，再核对`optionlib.md`与实际图示、定价资源。
2. 已录入时，读取对应正文，按用户关注点说明定义、构造、损益、关键条件、适用场景和风险。
3. 待录入时，说明缺失或待核验的具体环节；若正文或图示已存在，可按其明确标注为候选资料，不得把它当作正式入库结论。
