#!/usr/bin/env python3
"""Regenerate Designer's five checked-in HTML examples.

Examples are derived from the public renderer and the canonical fixture. They
...
def main() -> None:
    ...
    for output_type in ("report", "card", "quote"):
        artifact = render(DesignerInput(
            payload=payload, output_type=output_type, asset_mode="shared", output_dir=examples,
        ))
        (examples / f"{output_type}.html").write_text(artifact["html"], encoding="utf-8")
