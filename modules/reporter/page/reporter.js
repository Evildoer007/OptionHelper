(() => {
  ...
  const state = { catalog: null, source: null, selected: new Set(), ready: false };
  ...
  function clearSourceState() {
    state.catalog = null; state.source = null; state.selected.clear(); renderSources();
  ...
  function updateControls() {
    ...
    $('formatRule').textContent = format === 'pdf' ? 'PDF采用固定页式并隐藏阅读目录；若Designer缺少PDF依赖，将真实显示不可用。' : outputType === 'card' ? '简报不使用目录或损益图。' : '完整报告固定为连续A4正文，左侧提供阅读目录。';
  ...
  function renderSources() {
    ...
    select.disabled = !sources.length; state.source = activeSource(); state.selected.clear(); renderCandidates();
  }
  function renderCandidates() {
    ...
    list.innerHTML = (source.candidates || []).map(candidate => {
      ...
      const runs = ['payoff','pricing','backtest'].map(module => { const run = candidate.module_run_refs?.[module]; const status = moduleStatus(run); return `<div class="run-row" data-status="${esc(status)}"><strong>${({payoff:'Payoffer',pricing:'Pricer',backtest:'Backtester'})[module]}</strong><span>${esc(status)}</span></div>`; }).join('');
  ...
  async function generate() {
    ...
    const selection = { source_id: source.source_id, candidate_ids: [...state.selected], selected_modules: selectedModules(), delivery_mode: $('deliveryMode').value, output_type: type, format, html_report_layout: type === 'report' && format === 'html' ? 'continuous' : null, audience: $('audience').value, report_run_id: identifier('report'), metadata: { title: source.label || '场外衍生品研究报告' } };
