...
```text
ReportRequest v2 + ResultStorePort
  -> ContractReportUnit / ProductKnowledgeUnit
...
`run_manifest.json`记录请求哈希、ReportUnit文件和事实哈希、Designer输入与设计简报哈希、所有ModuleRunRef、产物路径/哈希、Designer设计系统版本、布局、实际交付文件哈希和真实运行状态。未运行、失败、不支持、部分完成和`legacy_unverified`均保留其原因，不能用默认图、示例数值或空白结论替代。
