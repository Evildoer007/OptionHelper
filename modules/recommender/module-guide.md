...
本模块不读取`references/`，不导入其他模块代码。业务流程只依赖AgentPort、KnowledgePort和ToolPort；AgentPort声明模型调用与独立子运行能力，App HTTP仅是ToolPort适配器，不是领域协议。自由流程按路由执行最小授权：闲聊无工具、知识仅Knowledger、数据仅Knowledger和DataFetcher、已有结果报告仅Reporter。相同工具请求在一次运行内只执行一次。缺少模型或所需端口时返回可审计的`unavailable/failed`，不生成事实或成功状态。
