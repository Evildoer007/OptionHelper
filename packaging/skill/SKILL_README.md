...
| 结构推荐与正式交付 | Recommender筛选结构；按需计算；Reporter→Designer交付 | 推荐本身不需要；首次取数或计算时需要 |

面向用户的估值、回测和报告只展示百分比。`S0Raw`仅用于真实价格与标准化合同换算，不作为面向用户字段；内部现金流、点数、金额和名义本金不得投影到页面、正式Tool、CSV或报告。

### 5.1当前对话使用方式

当前对话通过受控结构化入口运行，不需要用户填写JSON，用户也不需要手写Python或HTML。首次要求研究简报、完整研究报告或参考报价时，系统按需求完成推荐与必要计算，形成合同快照后直接交付。研究简报固定为结构推荐、推荐理由、关键合同条款、估值摘要、回测摘要、主要风险，无损益图；完整研究报告采用连续A4正文，固定为核心结论、结构推荐、合同参数、收益结构、估值定价、历史回测、风险提示，宽屏HTML提供左侧七章目录，PDF不显示目录。用户只要要求参考报价，当前对话默认决定应纳入的多个结构、标的、期限和参数版本；每一行完成必要定价后汇入同一份报价表，不要求用户逐行选择候选、期限、旧结果或拼接方式。用户明确指定结构、期限、条款或已保存结果时，系统按其指定覆盖对应部分。后续可复用用户明确选择的已保存运行结果。用户可随时改条款后只运行一个模块，形成新的合同快照；也可从旧版或新版快照直接生成任一交付。已有Card或Report需要换一种形式时，直接复用其保存的事实集重新渲染，不重跑分析；新的结构组合或Quote由Reporter冻结本次选择的事实。HTML和PDF均按固定顺序连续呈现。
