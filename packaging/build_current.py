...
def _verify_layout(stage: Path, version: str, platform: str, *, verify_zip_fn) -> None:
    installer = (
        ...
        if platform == "macos"
        else f"OptionHelper-{version}-Windows-x64.exe"
    )
...
def archive_deliveries(delivery_root: Path, versions_root: Path) -> list[Path]:
    ...
    for artifact in sorted(delivery_root.iterdir()):
        ...
        app = re.fullmatch(r"OptionHelper-(v[0-9][A-Za-z0-9._-]*)-(?:macOS-arm64\.dmg|Windows-x64\.exe|windows-x86_64(?:\.zip|-Setup\.exe))", artifact.name)
        match = skill or app
        if not match:
            continue
