...
try {
  if (!command || command === '--help' || command === 'help') {
    ...
    const config = migrateConfig(await readConfig(values.input));
    ...
    if (command === 'render' && validation.errors.length === 0) {
      ...
      emit({ ok: result.errors.length === 0, validation: result, output, config: result.config }, result.errors.length ? 1 : 0);
